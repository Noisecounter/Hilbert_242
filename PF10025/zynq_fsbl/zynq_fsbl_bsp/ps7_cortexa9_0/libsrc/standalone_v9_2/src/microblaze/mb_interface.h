 ...
 *@endcond
 */
extern void microblaze_register_handler(XInterruptHandler Handler,
					void *DataPtr); /* Register top level interrupt handler */
extern void microblaze_register_exception_handler(u32 ExceptionId, Xil_ExceptionHandler Handler,
	void *DataPtr); /* Register exception handler */
...
#define getfsl(val, id)         __asm volatile ("get\t%0,rfsl" stringify(id) : "=d" (val))
#define putfsl(val, id)         __asm volatile ("put\t%0,rfsl" stringify(id) :: "d" (val))

/* Non-blocking Data Read and Write to FSL no. id */
#define ngetfsl(val, id)        __asm volatile ("nget\t%0,rfsl" stringify(id) : "=d" (val))
#define nputfsl(val, id)        __asm volatile ("nput\t%0,rfsl" stringify(id) :: "d" (val))

/* Blocking Control Read and Write to FSL no. id */
#define cgetfsl(val, id)        __asm volatile ("cget\t%0,rfsl" stringify(id) : "=d" (val))
#define cputfsl(val, id)        __asm volatile ("cput\t%0,rfsl" stringify(id) :: "d" (val))

/* Non-blocking Control Read and Write to FSL no. id */
#define ncgetfsl(val, id)       __asm volatile ("ncget\t%0,rfsl" stringify(id) : "=d" (val))
#define ncputfsl(val, id)       __asm volatile ("ncput\t%0,rfsl" stringify(id) :: "d" (val))

/* Polling versions of FSL access macros. This makes the FSL access interruptible */
#define getfsl_interruptible(val, id)       __asm volatile ("\n1:\n\tnget\t%0,rfsl" stringify(id) "\n\t"   \
	"addic\tr18,r0,0\n\t"                \
	"bnei\tr18,1b\n"                     \
	: "=d" (val) :: "r18")

#define putfsl_interruptible(val, id)       __asm volatile ("\n1:\n\tnput\t%0,rfsl" stringify(id) "\n\t"   \
	"addic\tr18,r0,0\n\t"                \
	"bnei\tr18,1b\n"                     \
	:: "d" (val) : "r18")

#define cgetfsl_interruptible(val, id)      __asm volatile ("\n1:\n\tncget\t%0,rfsl" stringify(id) "\n\t"  \
	"addic\tr18,r0,0\n\t"                \
	"bnei\tr18,1b\n"                     \
	: "=d" (val) :: "r18")

#define cputfsl_interruptible(val, id)      __asm volatile ("\n1:\n\tncput\t%0,rfsl" stringify(id) "\n\t"  \
	"addic\tr18,r0,0\n\t"                \
	"bnei\tr18,1b\n"                     \
	:: "d" (val) : "r18")
/* FSL valid and error check macros. */
#define fsl_isinvalid(result)               __asm volatile ("addic\t%0,r0,0"  : "=d" (result))
#define fsl_iserror(error)                  __asm volatile ("mfs\t%0,rmsr\n\t"  \
